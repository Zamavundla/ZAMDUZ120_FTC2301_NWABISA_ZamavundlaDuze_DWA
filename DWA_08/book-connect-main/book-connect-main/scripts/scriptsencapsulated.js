...
/**
 * BookList class represents a book list application.
 */
class BookList {
  /**
   * Creates an instance of BookList.
   * @param {object} htmlSelectors - The HTML selectors for different sections of the application.
   */
  ...
  /**
   * Initializes the book list application.
   */
  init() {
    this.loadBooks();
   ...
   * Uses the instance's index variable to track the number of books loaded so far.
   * 
   ...
   */
  toggleSearch() {
    const searchOverlay = this.html.search.overlay;
    searchOverlay.classList.toggle('overlay__open');

    const searchInput = this.html.search.input;
    searchInput.focus();
    searchInput.value = '';
  }
   ...
   */
  addEventListeners() {
    this.html.list.button.addEventListener('click', this.loadBooks.bind(this));
    window.addEventListener('load', this.loadBooks.bind(this));
    this.html.list.button.addEventListener('click', this.handleBookPreview.bind(this));
    this.html.list.overlay.close.addEventListener('click', this.handleBookPreview.bind(this));
    this.html.search.button.addEventListener('click', this.toggleSearch.bind(this));
  }
}
...
const htmlSelectors = {
  list: {
    ...
    overlay: {
      active: document.querySelector('[data-list-overlay]'),
...
const bookList = new BookList(htmlSelectors);
bookList.init();
